		...
		if (hd == null) {
			...
			//if (logger.isDebugEnabled())
				...
				if(ElectionManager.getInstance().leaderNode != null)
				 logger.info("I am node "+ElectionManager.getInstance().getNodeId() +"The Acccepted leader is " + ElectionManager.getInstance().leaderNode);
	...
	public class CloseHeartListener implements ChannelFutureListener {
		...
		public void operationComplete(ChannelFuture future) throws Exception {
			if (outgoingHB.containsValue(heart)) {
				...
				if(ElectionManager.getInstance().leaderNode == heart.getNodeId()){
					logger.warn("Leader RIP");
