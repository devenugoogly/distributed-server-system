...
public class ServerHandler extends SimpleChannelInboundHandler<Request> {
	...
	public void channelRead0(ChannelHandlerContext ctx, Request req) throws Exception {
		...
		{
			logger.info("recieved data from "+req.getHeader().getClusterId());
			ConnectionManager.addinterClusterConnection(req.getHeader().getClusterId(), ctx.channel());
		}
	...
	public void channelInactive(ChannelHandlerContext ctx) throws Exception {
		System.out.println("Channel Inactive "+ctx.channel().isActive());
		ConnectionManager.removeClientConnection(ctx.channel());
	}
